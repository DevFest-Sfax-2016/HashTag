 ...
 */
public class UpdateOperationTask extends AsyncTask<String,Integer,Boolean> {
    Context context;
    Operation tmpOperation;
    private ProgressDialog pBar;
    private boolean statuts;

    public UpdateOperationTask(Context context, Operation tmpOperation) {
    ...
    protected Boolean doInBackground(String... params) {
        ...
        data.add(new BasicNameValuePair("datedebut",tmpOperation.getDatedebut()+""));

        JSONObject json = jParser.makeHttpRequest("http://"+ HostServer.IP+"/RestAPI.svc/UpdateOperation/"+tmpOperation.getNumoperation()+"/"+tmpOperation.getNumrobe()+"/"+tmpOperation.getCin()+"/"+tmpOperation.getNomclient()+"/"+tmpOperation.getAvance()+"/"+tmpOperation.getDatedebut(), "GET", data);
